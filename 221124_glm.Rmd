...
author: 
 ...
 - Gianluca Mastrantonio^[Politecnico di Torino, gianluca.mastrantonio@polito.it]
...
students %>%
  melt(measure.vars = c("write", "science", "math", "read"),
...
- R-squared cannot be computed (there are no residuals) and the
  ...
  - `Null deviance` represents the distance of this fit with the model
    which only has the intercept
  - `Residual deviance` compares the fit with a perfect model (with number
...
```{r}
head(general_glm$fitted.values)
```
```{r}
head(predict(general_glm, newdata = students, type = "response")) # probs
head(predict(general_glm, newdata = students)) # logit
```
...
eta <- predict(general_glm, newdata = students)
...
$$
X_{vg}\beta_{vg} = \log(\frac{\pi_{v}}{\pi_g})\,,\\
X_{ag}\beta_{ag} = \log(\frac{\pi_{a}}{\pi_g})\,,
$$
...
```{r}
bind_cols(pred, students) %>%
  melt(measure.vars = 1:3,
       variable.name = "predProg",
       value.name = "prediction") -> pred_stud_long
pred_stud_long %>%
  ggplot() +
