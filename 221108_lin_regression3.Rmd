...
library(tibble)
n <- 100
synth <- tibble(x = seq(from = 1, to = 30, length.out = n),
...
ggplot(log_lm, 
       mapping = aes(`log(x)`, y)) + # notice the backticks!
  geom_point() +
  geom_smooth(method = "lm", color = "red")
```
...
```{r}
wide_golf <- read_csv("./datasets/golfer.csv", col_types = cols(
  golfer = col_factor(),
  A = col_double(),
  B = col_double(),
  C = col_double(),
...
and we can copare it to a smaller model with ANOVA test.
