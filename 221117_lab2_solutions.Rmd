...
# Linear regression lab (with solutions)
...
The predictors are ``r colnames(insulate %>% select(-cons))``.
...
ggplot() +
  ...
  geom_abline(mapping = aes(linetype = "complete", color = c("after", "before"),
                            intercept = c(additive_lm$coefficients[1] + additive_lm$coefficients[2],
...
ggplot() +
  ...
  geom_ribbon(aes(temp, ymin = lwr_pred, ymax = upr_pred, fill = "prediction"),
              data = new_data, alpha = .5) + # pred intervals
  geom_ribbon(aes(temp, ymin = lwr_conf, ymax = upr_conf, fill = "confidence"),
              data = new_data, alpha = .5) # conf intervals
```
